...
if cat /etc/os-release | grep PRETTY_NAME | grep "Ubuntu 20.04"; then
    ...
    /usr/bin/wget --no-check-certificate -O /etc/squid/squid.conf https://raw.githubusercontent.com/minnie1311/Python3/main/squid.conf
    ...
    /usr/bin/wget --no-check-certificate -O /etc/squid3/squid.conf https://raw.githubusercontent.com/minnie1311/Python3/main/squid.conf
    /sbin/iptables -I INPUT -p tcp --dport 443 -j ACCEPT
    /sbin/iptables-save
